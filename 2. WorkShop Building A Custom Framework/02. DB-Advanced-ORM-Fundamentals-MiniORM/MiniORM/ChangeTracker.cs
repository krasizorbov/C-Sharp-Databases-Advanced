...
namespace MiniORM
{
	internal class ChangeTracker<T> where T:class, new()
    {
        ...
        private static List<T> CloneEntities(IEnumerable<T> entities)
        {
            ...
            var propertiesToClone = typeof(T).GetProperties().Where(pi => DbContext.AllowdSqlTypes.Contains(pi.PropertyType)).ToArray();
        ...
        {
            var monitoredProperties = typeof(T).GetProperties().Where(pi => DbContext.AllowdSqlTypes.Contains(pi.PropertyType));
