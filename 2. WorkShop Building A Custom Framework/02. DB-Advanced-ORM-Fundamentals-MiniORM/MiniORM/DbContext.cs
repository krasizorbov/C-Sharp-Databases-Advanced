...
namespace MiniORM
{
	public abstract class DbContext
    {
        ...
        internal static readonly Type[] AllowdSqlTypes =
        {
            typeof(string),
            typeof(int),
            typeof(uint),
        ...
        private void MapRelations<TEntity>(DbSet<TEntity> dbSet) where TEntity : class, new()
        {
            ...
            var collections = entityType.GetProperties()
                .Where(pi => pi.PropertyType.IsGenericType && pi.PropertyType
                .GetGenericTypeDefinition() == typeof(ICollection<>)).ToArray();
        ...
        {
            var tableName = ((TableAttribute)tableType
        ...
        private string[] GetEntityColumnNames(Type table)
        {
            ...
            var columns = table.GetProperties().Where(pi => dbColumns
            .Contains(pi.Name) && !pi.HasAttribute<NotMappedAttribute>() && AllowdSqlTypes
            .Contains(pi.PropertyType)).Select(pi => pi.Name).ToArray();
            return columns;
        }
    }
}