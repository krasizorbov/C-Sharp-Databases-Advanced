    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (dbCon)
            {
                ...
                {
                    int townsCount = (int)new SqlCommand($"SELECT COUNT(*) FROM Towns WHERE {countryId} = CountryCode", dbCon).ExecuteScalar();

                    SqlDataReader reader = new SqlCommand($"SELECT * FROM Towns WHERE CountryCode = {countryId}", dbCon).ExecuteReader();
